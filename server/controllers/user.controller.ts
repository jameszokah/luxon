...
export const updateAdminRole = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { email, role } = req.body;
      const isAdminExist = await userModel.findOne({ email, role: "admin" });
      if (isAdminExist) {
        const id = isAdminExist._id;
        updateUserRoleService(res, id, role);
      } else {
        res.status(400).json({
          success: false,
...
export const deleteAdmin = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
